...
public class MemberController {
	...
	public String afterRegister(@Valid @ModelAttribute("donor") Donor donor, BindingResult res,
 		...
 		MultipartFile userImg = donor.getUserImage();	
		...
		donor.getEligible().setEligible(true);
		donorService.saveDonor(donor);
		redirectattributes.addFlashAttribute(donor);
		
		if (userImg!=null && !userImg.isEmpty()) {
		       try {
		    	   userImg.transferTo(new File(rootDirectory+ "resources\\images\\"+ donor.getUsername() + ".png"));
		       } catch (Exception e) {
